...
const ShopifyUpload = () => {
    ...
    const processCSVData = (rows) => {
        ...
        rows.forEach((row) => {
            // const styleNumber = extractStyleNumber(row["Variant SKU"] || 22222);
            const styleNumber = row["Variant SKU"]?.split("-")[0] ? row["Variant SKU"]?.split("-")[0]?.split(" ")[0] : 22222
            const channel = "Shopify";
            const productId = row["Handle"]?.trim() || '';
            const price = row["Variant Compare At Price"] ? parseFloat(row["Variant Compare At Price"]) : 11111;
            ...
            if (!uniqueMap.has(key)) {
                ...
                processedRecords.push({
                    ...
                    channel: "Shopify",
                    product_id: productId || "handle not provided",
                    price: price || 11111,
    ...
    const uploadToServer = async () => {
        if (records.length === 0) {
            setUploadStatus({
                type: 'warning',
                message: 'No valid Shopify records found to upload'
            });
            return;
        }
                ...
                <div className={`border-3 border-dashed rounded-2xl p-10 text-center transition-all ${file
                    ...
                    <div className="mb-6">
                        <h3 className="text-xl font-medium text-gray-900 mb-2">
                            {file ? 'File Selected ✓' : 'Drop SHOPIFY CSV file here'}
                        </h3>
                        <p className="text-gray-500">
                            {file
                                ? file.name
                                : 'or click to browse shopify file'
                            }
                        </p>
                        {file && (
