...
const NykaaUpload = () => {
    ...
    const processCSVData = (rows) => {
        ...
        rows.forEach((row) => {
            ...
            const styleNumber = row["Vendor SKU Code"]?.split("-")[0] || 0
            const channel = "Nykaa";
            const productId = row["Parent Product Id"]?.trim() || '';
            const price = row["MRP"] || 0;
                ...
                <div className={`border-3 border-dashed rounded-2xl p-10 text-center transition-all ${file
                    ...
                    <div className="mb-6">
                        <h3 className="text-xl font-medium text-gray-900 mb-2">
                            {file ? 'File Selected ✓' : 'Drop Nykaa CSV file here'}
                        </h3>
                        <p className="text-gray-500">
                            {file
                                ? file.name
                                : 'or click to browse Nykaa file'
                            }
                        </p>
                        {file && (
